...
public class PermissionsValidator {
    ...
    public static boolean validateTableName(String tableName, EnumSet<Permissions> perms) 
        ...
        if (perms.contains(NOT_NULL) && tableName == null) {
            throw new RuntimeException("Table name hasn't been specified");
        }
        return true;
    }
    ...
    public static boolean validateDbFolder(String dbFolder, EnumSet<Permissions> perms)
        ...
        if (perms.contains(NOT_NULL) && dbFolder == null) {
            throw new RuntimeException("Database name hasn't been specified");
        }
        return true;
    }
    ...
    public static boolean validate(String fileName, 
        ...
        if (perms.contains(NOT_NULL) && fileName == null) {
            throw new RuntimeException("File name hasn't been specified");
        }
        File file = new File(fileName);
        if (perms.contains(EXISTS) && !file.exists()) {
            return false;
        }
        if ((perms.contains(CREATE_FILE_IF_NOT_EXISTS) || perms.contains(CREATE_DIRECTORY_IF_NOT_EXISTS))
                && !file.exists()) {
            File parentFile = file.getCanonicalFile().getParentFile();
            if (!parentFile.canWrite()) {
                throw new RuntimeException(fileName + ": don't have permission to create the file");
            }
        ...
        if (perms.contains(CAN_READ) && !file.canRead()) {
            throw new RuntimeException(file.getPath() + ": don't have permission to read the file");
        }
        if (perms.contains(CAN_WRITE) && !file.canWrite()) {
            throw new RuntimeException(file.getPath() + ": don't have permission to write the file");
        }
        if (perms.contains(IS_FILE) && !file.isFile()) {
            throw new RuntimeException(file.getPath() + ": isn't a normal file");
        }
        if (perms.contains(IS_DIRECTORY) && !file.isDirectory()) {
            throw new RuntimeException(file.getPath() + ": isn't a directory");
        }
        return true;
    }
}
